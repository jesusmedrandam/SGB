...
const line=z.object({animalId:z.uuid().optional(),productName:z.string().trim().min(1).max(180).optional(),
  ...
  .refine(value=>Boolean(value.animalId)!==Boolean(value.productName))
...
const record=z.object({kind:z.enum(['SALE','PURCHASE']),tradedOn:z.iso.date(),
  counterpartyName:z.string().trim().min(1).max(180),
